...
class TrainerBase:
    ...
    def train(self):
        ...
        for ii in tqdm(range(self.iters_start, self.configs.train.iterations), desc="Training Progress"):
    ...
    def logging_image(self, im_tensor, tag, phase, add_global_step=False, nrow=8):
        ...
        im_tensor_real = im_tensor[:, 0:1, :, :] # b x 1 x h x w
        im_tensor_imag = im_tensor[:, 1:2, :, :] # b x 1 x h x w

        im_tensor_complex = im_tensor_real + 1j*im_tensor_imag 
        im_tensor_abs = torch.abs(im_tensor_complex) # b x 1 x h x w
        im_tensor_phase = torch.angle(im_tensor_complex)

        assert self.tf_logging or self.local_logging
        im_tensor_abs = vutils.make_grid(im_tensor_abs, nrow=nrow, normalize=True, scale_each=True) 
        im_tensor_phase = vutils.make_grid(im_tensor_phase, nrow=nrow, normalize=True, scale_each=True) 

        if self.local_logging:
            im_path_amp = str(self.image_dir / phase / f"{tag}-amp-{self.log_step_img[phase]}.png")
            im_path_phase = str(self.image_dir / phase / f"{tag}-phase-{self.log_step_img[phase]}.png")
    
            im_np_abs = im_tensor_abs.cpu().permute(1,2,0).numpy()
            im_np_phase = im_tensor_phase.cpu().permute(1,2,0).numpy()

            im_np_abs = im_np_abs[:, :, 0]
            im_np_abs_3channel = np.stack([im_np_abs, im_np_abs, im_np_abs], axis=2)

            im_np_phase = im_np_phase[:, :, 0]
            im_np_phase_3channel = np.stack([im_np_phase, im_np_phase, im_np_phase], axis=2)

            util_image.imwrite(im_np_abs_3channel, im_path_amp)
            util_image.imwrite(im_np_phase_3channel, im_path_phase)
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            if 'gt' in data:
                ...
                self.logging_metric(mean_psnr, tag='PSNR', phase=phase, add_global_step=True)
